...
class NpmLinkManager extends BaseLinkManager_1.BaseLinkManager {
    async _linkProjectsAsync() {
        const npmPackage = await node_core_library_1.LegacyAdapters.convertCallbackToPromise(read_package_tree_1.default, this._rushConfiguration.commonTempFolder);
            ...
            for (const dependency of commonPackage.dependencies) {
                ...
                if (matchedRushPackage) {
                    ...
                    if (cyclicSubtreeRoot) {
                        ...
                        // Is this wise?)
                        // eslint-disable-next-line no-console
                        console.log(terminal_1.Colorize.yellow(`Rush will not locally link ${dependency.name} for ${localPackage.name}` +
