...
class AsyncOperationQueue {
    ...
    complete(record) {
        ...
        if (record.status !== OperationStatus_1.OperationStatus.Failure && record.status !== OperationStatus_1.OperationStatus.Blocked) {
            ...
            for (const item of record.consumers) {
                ...
                if (item.dependencies.delete(record) &&
                    ...
                    item.status === OperationStatus_1.OperationStatus.Waiting) {
                    item.status = OperationStatus_1.OperationStatus.Ready;
                }
            }
        }
        this.assignOperations();
    ...
    assignOperations() {
        ...
        for (let i = queue.length - 1; waitingIterators.length > 0 && i >= 0; i--) {
            ...
            if (record.status === OperationStatus_1.OperationStatus.Blocked ||
                record.status === OperationStatus_1.OperationStatus.Skipped ||
                record.status === OperationStatus_1.OperationStatus.Success ||
                record.status === OperationStatus_1.OperationStatus.SuccessWithWarning ||
                record.status === OperationStatus_1.OperationStatus.FromCache ||
                record.status === OperationStatus_1.OperationStatus.NoOp ||
                record.status === OperationStatus_1.OperationStatus.Failure) {
                // It shouldn't be on the queue, remove it
                queue.splice(i, 1);
            }
            else if (record.status === OperationStatus_1.OperationStatus.Queued || record.status === OperationStatus_1.OperationStatus.Executing) {
                // This operation is currently executing
                // next one plz :)
            }
            else if (record.status === OperationStatus_1.OperationStatus.Waiting) {
                // This operation is not yet ready to be executed
                // next one plz :)
                continue;
            }
            else if (record.status !== OperationStatus_1.OperationStatus.Ready) {
                // Sanity check
                ...
                record.status = OperationStatus_1.OperationStatus.Queued;
    ...
    if (dependencyChain.has(operation)) {
        throw new Error('A cyclic dependency was encountered:\n  ' +
            [...dependencyChain, operation]
                ...
                .reverse()
                .join('\n  -> ') +
            `\nConsider using the decoupledLocalDependencies option in ${RushConstants_1.RushConstants.rushJsonFilename}.`);
    }
