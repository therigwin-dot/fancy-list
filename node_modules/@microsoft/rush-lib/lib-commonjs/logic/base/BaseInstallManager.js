...
class BaseInstallManager {
    ...
    async doInstallAsync() {
        ...
        if (resolutionOnly ||
            ...
            if (this.options.allowShrinkwrapUpdates && !shrinkwrapIsUpToDate) {
                ...
                const shrinkwrapFile = ShrinkwrapFileFactory_1.ShrinkwrapFileFactory.getShrinkwrapFile(this.rushConfiguration.packageManager, committedShrinkwrapFileName);
    ...
    async prepareAsync(subspace, variant, projectImpactGraphGenerator) {
        ...
        if (!this.options.fullUpgrade) {
            const committedShrinkwrapFileName = subspace.getCommittedShrinkwrapFilePath(variant);
            try {
                shrinkwrapFile = ShrinkwrapFileFactory_1.ShrinkwrapFileFactory.getShrinkwrapFile(this.rushConfiguration.packageManager, committedShrinkwrapFileName);
            }
            catch (ex) {
                terminal.writeLine();
...
`;
    ...
    pushConfigurationArgs(args, options, subspace) {
        ...
        if (this.rushConfiguration.packageManager === 'npm') {
            ...
            */
            const isAutoInstallPeersInNpmrc = (0, npmrcUtilities_1.isVariableSetInNpmrcFile)(subspace.getSubspaceConfigFolderPath(), 'auto-install-peers', this.rushConfiguration.isPnpm);
            ...
            */
            const isResolutionModeInNpmrc = (0, npmrcUtilities_1.isVariableSetInNpmrcFile)(subspace.getSubspaceConfigFolderPath(), 'resolution-mode', this.rushConfiguration.isPnpm);
