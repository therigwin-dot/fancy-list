...
const deploy_scenario_schema_json_1 = __importDefault(require("../../schemas/deploy-scenario.schema.json"));
...
class DeployScenarioConfiguration {
    ...
    static loadFromFile(terminal, scenarioFilePath, rushConfiguration) {
        ...
        terminal.writeLine(terminal_1.Colorize.cyan(`Loading deployment scenario: ${scenarioFilePath}`));
            ...
            for (const additionalProjectsToInclude of projectSetting.additionalProjectsToInclude || []) {
                if (!rushConfiguration.getProjectByName(projectSetting.projectName)) {
                    throw new Error(`The "additionalProjectsToInclude" setting refers to the` +
                        ` project name "${additionalProjectsToInclude}" which was not found in ${RushConstants_1.RushConstants.rushJsonFilename}`);
                }
            }
...
DeployScenarioConfiguration._jsonSchema = node_core_library_1.JsonSchema.fromLoadedObject(deploy_scenario_schema_json_1.default);
