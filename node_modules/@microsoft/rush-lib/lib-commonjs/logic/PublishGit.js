...
class PublishGit {
    ...
    async checkoutAsync(branchName, createBranch = false) {
        ...
        await PublishUtilities_1.PublishUtilities.execCommandAsync(!!this._targetBranch, this._gitPath, params);
    }
    async mergeAsync(branchName, verify = false) {
        await PublishUtilities_1.PublishUtilities.execCommandAsync(!!this._targetBranch, this._gitPath, [
            'merge',
            branchName,
            '--no-edit',
            ...(verify ? [] : ['--no-verify'])
    ...
    async fetchAsync() {
        await PublishUtilities_1.PublishUtilities.execCommandAsync(!!this._targetBranch, this._gitPath, ['fetch', 'origin']);
    }
    async addChangesAsync(pathspec, workingDirectory) {
        const files = pathspec ? pathspec : '.';
        await PublishUtilities_1.PublishUtilities.execCommandAsync(!!this._targetBranch, this._gitPath, ['add', files], workingDirectory ? workingDirectory : process.cwd());
    }
    async addTagAsync(shouldExecute, packageName, packageVersion, commitId, preReleaseName) {
        ...
        await PublishUtilities_1.PublishUtilities.execCommandAsync(!!this._targetBranch && shouldExecute, this._gitPath, [
            'tag',
            '-a',
    ...
    async hasTagAsync(packageConfig) {
        ...
        const tagOutput = (await Utilities_1.Utilities.executeCommandAndCaptureOutputAsync(this._gitPath, ['tag', '-l', tagName], packageConfig.projectFolder, PublishUtilities_1.PublishUtilities.getEnvArgs(), true)).replace(/(\r\n|\n|\r)/gm, '');
