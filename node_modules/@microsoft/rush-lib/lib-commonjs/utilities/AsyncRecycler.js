...
class AsyncRecycler {
    ...
    moveFolder(folderPath) {
        ...
        Utilities_1.Utilities.retryUntilTimeout(() => this._renameOrRecurseInFolder(folderPath), maxWaitTimeMs, (e) => new Error(`Error: ${e}\nOften this is caused by a file lock from a process like the virus scanner.`), 'recycleFolder');
    }
    /**
    ...
    moveAllItemsInFolder(folderPath, membersToExclude) {
        ...
        for (const dirent of node_core_library_1.FileSystem.readFolderItems(resolvedFolderPath)) {
            ...
            if (!excludeSet.has(normalizedMemberName)) {
                ...
                if (dirent.isDirectory()) {
                    ...
                    node_core_library_1.FileSystem.deleteFile(absolutePath);
                }
            }
        }
    }
    ...
    async startDeleteAllAsync() {
        ...
        if (os.platform() === 'win32') {
            ...
            let folderItemNames = [];
            try {
                folderItemNames = await node_core_library_1.FileSystem.readFolderItemNamesAsync(this.recyclerFolder);
            }
            catch (e) {
    ...
    _renameOrRecurseInFolder(folderPath) {
        ...
        const children = node_core_library_1.FileSystem.readFolderItems(folderPath);
        for (const child of children) {
            ...
            if (child.isDirectory()) {
                ...
                node_core_library_1.FileSystem.deleteFile(absoluteChild);
            }
        }
        // Yes, this is a folder. The API deletes empty folders, too.
        node_core_library_1.FileSystem.deleteFile(folderPath);
    }
}
