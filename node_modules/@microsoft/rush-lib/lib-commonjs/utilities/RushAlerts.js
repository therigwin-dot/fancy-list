...
exports.RushAlerts = void 0;
const terminal_1 = require("@rushstack/terminal");
const node_core_library_1 = require("@rushstack/node-core-library");
const rush_alerts_schema_json_1 = __importDefault(require("../schemas/rush-alerts.schema.json"));
...
class RushAlerts {
    ...
    static async loadFromConfigurationAsync(rushConfiguration, terminal) {
        const rushAlertsStateFilePath = `${rushConfiguration.commonTempFolder}/${RushConstants_1.RushConstants.rushAlertsConfigFilename}`;
        const rushAlertsConfigFilePath = `${rushConfiguration.commonRushConfigFolder}/${RushConstants_1.RushConstants.rushAlertsConfigFilename}`;
        ...
        const [isRushAlertsStateFileExists, isRushAlertsConfigFileExists] = await Promise.all([
            node_core_library_1.FileSystem.existsAsync(rushAlertsStateFilePath),
            node_core_library_1.FileSystem.existsAsync(rushAlertsConfigFilePath)
        ]);
        const [rushAlertsConfig, rushAlertsState] = await Promise.all([
            isRushAlertsConfigFileExists
                ? node_core_library_1.JsonFile.loadAndValidateAsync(rushAlertsConfigFilePath, node_core_library_1.JsonSchema.fromLoadedObject(rush_alerts_schema_json_1.default))
                : undefined,
            isRushAlertsStateFileExists
                ? node_core_library_1.JsonFile.loadAsync(rushAlertsStateFilePath, { jsonSyntax: node_core_library_1.JsonSyntax.JsonWithComments })
                : undefined
        ]);
    ...
    _printAlerts(alerts, status) {
        ...
        switch (status) {
            case 'active':
            case 'inactive':
                this._terminal.writeLine(terminal_1.Colorize.yellow(`The following alerts are currently ${status}:`));
                break;
            case 'snoozed':
                this._terminal.writeLine(terminal_1.Colorize.yellow('The following alerts are currently active but snoozed:'));
                break;
        }
        alerts.forEach(({ title }) => {
            this._terminal.writeLine(terminal_1.Colorize.green(`"${title}"`));
        });
        this._terminal.writeLine();
    }
    ...
    async _isAlertValidAsync(alert) {
        ...
        if (conditionScript) {
            ...
            if (!(await node_core_library_1.FileSystem.existsAsync(conditionScriptPath))) {
    ...
    _printMessageInBoxStyle(alert) {
        ...
        lineLength = Math.min(lineLength, ((_a = terminal_1.PrintUtilities.getConsoleWidth()) !== null && _a !== void 0 ? _a : 80) - PADDING, 80 - PADDING);
        // ...and the width needs to be at least 40 characters...
        lineLength = Math.max(lineLength, 40 - PADDING);
        const lines = [
            ...terminal_1.PrintUtilities.wrapWordsToLines(boxTitle, lineLength).map((x) => terminal_1.Colorize.bold(x.padEnd(lineLength))),
            '',
            ...terminal_1.PrintUtilities.wrapWordsToLines(boxMessage, lineLength).map((x) => x.padEnd(lineLength))
        ];
        if (boxDetails) {
            lines.push('', ...terminal_1.PrintUtilities.wrapWordsToLines(boxDetails, lineLength).map((x) => terminal_1.Colorize.cyan(x.padEnd(lineLength))));
        }
        // Print the box
    ...
    async _writeRushAlertStateAsync() {
        await node_core_library_1.JsonFile.saveAsync(this._rushAlertsState, this.rushAlertsStateFilePath, {
            ignoreUndefinedValues: true,
            headerComment: '// THIS FILE IS MACHINE-GENERATED -- DO NOT MODIFY',
            jsonSyntax: node_core_library_1.JsonSyntax.JsonWithComments
        });
    }
}
exports.RushAlerts = RushAlerts;
