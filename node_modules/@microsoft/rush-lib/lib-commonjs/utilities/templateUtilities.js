...
exports.copyTemplateFileAsync = void 0;
...
async function copyTemplateFileAsync(sourcePath, destinationPath, overwrite, demo = false) {
    const destinationFileExists = await node_core_library_1.FileSystem.existsAsync(destinationPath);
    if (!overwrite) {
        if (destinationFileExists) {
            // eslint-disable-next-line no-console
            console.log(terminal_1.Colorize.yellow('Not overwriting already existing file: ') + destinationPath);
            return;
        }
    }
    if (destinationFileExists) {
        // eslint-disable-next-line no-console
        console.log(terminal_1.Colorize.yellow(`Overwriting: ${destinationPath}`));
    }
    else {
        // eslint-disable-next-line no-console
    ...
    const outputLines = [];
    const lines = (await node_core_library_1.FileSystem.readFileAsync(sourcePath, { convertLineEndings: node_core_library_1.NewlineKind.Lf })).split('\n');
    ...
    // Write the output
    await node_core_library_1.FileSystem.writeFileAsync(destinationPath, outputLines.join('\n'), {
        ensureFolderExists: true
    });
}
exports.copyTemplateFileAsync = copyTemplateFileAsync;
