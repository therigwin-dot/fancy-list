...
exports.SearchListPrompt = void 0;
...
class SearchListPrompt extends base_1.default {
    constructor(question, readline, answers) {
        ...
        this._paginator = new paginator_1.default(this.screen);
    }
    _run(callback) {
        this.done = callback;
        ...
        // eslint-disable-next-line @typescript-eslint/typedef
        const validation = this.handleSubmitEvents(events.line.pipe((0, operators_1.map)(this._getCurrentValue.bind(this))));
    ...
    render(error) {
        ...
        if (this._firstRender) {
            message += terminal_1.Colorize.dim(' (Use arrow keys)');
        }
        // Render choices or answer depending on the state
        if (this.status === 'answered') {
            message += terminal_1.Colorize.cyan(this.opt.choices.getChoice(this._selected).short);
        }
        else {
            ...
            message += `\n${terminal_1.Colorize.white(terminal_1.Colorize.bold('Start typing to filter:'))} ${terminal_1.Colorize.cyan(this._query)}`;
            // @ts-expect-error Types are wrong
            message += '\n' + this._paginator.paginate(choicesStr, realIndexPosition, this.opt.pageSize);
        }
        if (error) {
            bottomContent = terminal_1.Colorize.red('>> ') + error;
        }
        this.screen.render(message, bottomContent);
    }
}
exports.SearchListPrompt = SearchListPrompt;
function listRender(choices, pointer) {
    ...
    choices.forEach((choice, i) => {
        ...
        if (!choice.disabled) {
            ...
            if (i === pointer) {
                output += terminal_1.Colorize.cyan(figures_1.default.pointer + line);
            }
            else {
                output += ' ' + line;
            }
