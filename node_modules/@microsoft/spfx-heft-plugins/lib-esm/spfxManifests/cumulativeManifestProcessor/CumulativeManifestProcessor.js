 ...
 */ export class CumulativeManifestProcessor {
   ...
   */ async generateCumulativeManifestAsync(debugManifests, manifestsJsFileMetadata, discoveryMode) {
    ...
    _discoverManifestsInner(packagePath, forceSearchMode, options, canSkipPackage) {
        ...
        const tempPath = `${packagePath}/${this._options.tempFolderName || DEFAULT_TEMP_FOLDER}`;
        ...
        if (!foundCumulativeManifestsFile) {
            ...
            try {
                for (const targetFilename of FileSystem.readFolderItemNames(targetFolderPath)){
                    if (targetFilename.match(manifestFileRegex)) {
                        const targetFilePath = `${targetFolderPath}/${targetFilename}`;
                        try {
                            const fileStats = FileSystem.getStatistics(targetFilePath);
                            ...
                            const manifestData = cloneDeep(JsonFile.load(targetFilePath));
