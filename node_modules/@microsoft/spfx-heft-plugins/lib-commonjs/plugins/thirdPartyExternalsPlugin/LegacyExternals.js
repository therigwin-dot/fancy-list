...
class LegacyExternals {
    ...
    async _collectResourcesForKeyAsync(rawPattern) {
        ...
        if (!pattern.match(LOCALE_TOKEN_REGEXP)) {
            ...
            const fullPath = _path.join(this._buildFolder, pattern);
            ...
            if (!files || files.length === 0) {
                ...
                const normalizedPattern = _path.posix.resolve('/', pattern).substr(1);
                // eslint-disable-next-line @rushstack/security/no-unsafe-regexp
                ...
                for (const file of files){
                    ...
                    if (!match) {
                        ...
                        result[locale] = _path.join(this._buildFolder, file);
                    }
                }
                return result;
            }
        ...
        if (!result) {
            ...
            if (localizedFiles) {
                ...
                for (const [localeName, filePath] of Object.entries(localizedFiles)){
                    ...
                    assetFilenameParts.push(_path.extname(filePath));
                ...
                const includeExternalAsync = async (name, externalModule)=>{
                    ...
                    if (/^https?\:/.test(externalPath)) {
                        ...
                        let fileContents;
                        const externalFullPath = _path.join(this._buildFolder, externalPath);
                        try {
                        ...
                        assetFilenameParts.push(_path.extname(externalPath));
                        const assetFilename = assetFilenameParts.join('');
