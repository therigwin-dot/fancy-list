...
const _moduleminifier = require("@rushstack/module-minifier");
...
class WebpackConfigurationGenerator {
    static async generateWebpackConfigurationAsync(options) {
        ...
        const distFolderName = _path.relative(buildFolder, outputFolder).replace(/^\.[\\\/]/, '');
        const tempFolderName = _path.relative(buildFolder, tempFolder).replace(/^\.[\\\/]/, '');
        ...
        for (const [bundleName, configEntry] of Object.entries(configBundleEntries)){
            ...
            for (const componentEntry of configEntry.components){
                const manifestData = await _nodecorelibrary.JsonFile.loadAsync(_path.resolve(options.folders.buildFolder, componentEntry.manifest));
                ...
                bundleEntry.components[manifestData.id] = {
                    ...componentEntry,
                    manifestData,
                    manifestPath: _path.join(options.folders.buildFolder, componentEntry.manifest),
        ...
        if (options.emitStats) {
            plugins.push(new _WebpackStatsPlugin.WebpackStatsPlugin({
                dropPath: _path.resolve(options.folders.releaseFolder, 'webpack-stats', `${_path.basename(options.folders.buildFolder)}.stats.json`)
            }));
        }
        ...
        let serveConfig = {};
        try {
            serveConfig = await _nodecorelibrary.JsonFile.loadAsync(_path.join(options.folders.buildFolder, 'config', 'serve.json'));
        } catch (e) {
        ...
        let cdnBaseUrl;
        try {
            const writeManifestsConfig = await _nodecorelibrary.JsonFile.loadAsync(_path.join(options.folders.buildFolder, 'config', 'write-manifests.json'));
        ...
        plugins.push(new _ManifestPlugin.ManifestPlugin({
            ...
            componentDependenciesAuditDropPath: _path.join(options.folders.releaseFolder, 'component-dependency-audit', `${_path.basename(options.folders.buildFolder)}.json`),
        ...
        if (options.production) {
            ...
            config.optimization.minimizer = [
                new _webpack5moduleminifierplugin.ModuleMinifierPlugin({
                    minifier: new _moduleminifier.WorkerPoolMinifier({
                        // Cap the number of worker processes to avoid CPU saturation.
                        maxThreads: Math.min(8, _os.cpus().length),
                        terserOptions: {
                            compress: {
                                passes: 3
                            },
   ...
   */ static async _generateBundleEntrypointFileAsync(options) {
        ...
        for (const bundleComponent of Object.values(options.bundleEntry.components)){
            ...
            const relativeEntrypointPath = _path.relative(entrypointFileFolderPath, fullEntrypointPath);
    ...
    static _requireResolveWithCorrectCase(packageName) {
        const resolveResult = _nodecorelibrary.Import.resolvePackage({
            ...
            getRealPath: (filePath)=>{
                try {
                    return _fs.realpathSync.native(filePath);
                } catch (error) {
        ...
        if (configJson.bundles) {
            ...
            for (const [, bundleEntry] of Object.entries(configJson.bundles)){
                for (const component of bundleEntry.components){
                    ...
                    const manifestPath = _path.resolve(_path.join(rootPath, component.manifest)).toUpperCase();
